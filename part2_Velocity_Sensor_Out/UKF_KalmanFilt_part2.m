...
addpath('../data')
datasetNum = 1; % CHANGE THIS VARIABLE TO CHANGE DATASET_NUM %It specifies the dataset number to be used
...
covarPrev = 0.01*eye(15); % Covariance constant %It initilizes the covariance matrix to be an Identity matrix of size 15x15
...
for i = 1:length(sampledTime) % This is For loop that iterates over each time step in the sampledTime
    ...
    currTime = proj2Data.time(i); %To extract the current timestamp from the sampleddata 
    ...
    % The Update step
    [uCurr,covar_curr] = upd_step(vel(i,:),covarEst,uEst,angVel2(i,:)); % To updates the state and covariance based on the measurement data and the predicted state and covariance

    uPrev = uCurr; % To update the preious state for the next iteration
    covarPrev = covar_curr; % To update the preious state for the next iteration

    savedStates(:,i) = uCurr(:); %To save current state at time step i
    
end

plotData(savedStates, sampledTime, sampledVicon, 2, datasetNum); % Plot the saved states against time along with the sampledVicon data
